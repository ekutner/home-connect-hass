...
async def async_setup_entry(
    ...
    async_add_entities(
        [HomeConnectRefreshButton(homeconnect), HomeConnecDebugButton(homeconnect)]
    )
...
class StartButton(EntityBase, ButtonEntity):
            ...
            and (
                (
                    ...
                    and (
                        ...
                        and self._appliance.selected_program.key
                        in self._appliance.available_programs
                    )
                )
                or (
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            if (
                op_state
                and op_state.value == "BSH.Common.EnumType.OperationState.Ready"
            ):
    ...
    def available(self) -> bool:
        return (
            ...
            and (
                "BSH.Common.Status.RemoteControlStartAllowed"
                not in self._appliance.status
                or self._appliance.status["BSH.Common.Status.RemoteControlStartAllowed"]
            )
        )
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            if ex.error_description:
                ...
                raise HomeAssistantError(
                    f"Failed to stop the selected program ({ex.code})"
                )
...
class HomeConnectRefreshButton(ButtonEntity):
    ...
    attr_unique_id = "homeconnect_refresh"
    attr_name = "Home Connect Refresh"
    attr_icon = "mdi:cloud-refresh"
    attr_device_info = HOME_CONNECT_DEVICE
    attr_available = True
...
class HomeConnecDebugButton(ButtonEntity):
    ...
    attr_device_info = HOME_CONNECT_DEVICE
    attr_unique_id = "homeconnect_debug"
    attr_name = "Home Connect Debug Info"
    attr_icon = "mdi:bug-check"
    attr_available = True
