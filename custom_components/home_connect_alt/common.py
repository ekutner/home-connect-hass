...
class EntityBase(ABC):
    ...
    def name_ext(self) -> str | None:
        """
        Provide the suffix of the name,
        can be be overriden by sub-classes to provide a custom or translated display name.
        """
        return None
    ...
    def name(self) -> str:
        ...
        if (
            self._conf
            and CONF_NAME_TEMPLATE in self._conf
            and self._conf[CONF_NAME_TEMPLATE]
        ):
        ...
        appliance_name = (
            self._appliance.name if self._appliance.name else self._appliance.type
        )
        name = self.name_ext if self.name_ext else self.pretty_enum(self._key)
        return (
            template.replace("$brand", self._appliance.brand)
...
class InteractiveEntityBase(EntityBase):
    ...
    async def async_added_to_hass(self):
        ...
        if self._key != "BSH.Common.Status.RemoteControlActive":
            self._appliance.register_callback(
                self.async_on_update, "BSH.Common.Status.RemoteControlActive"
            )

    async def async_will_remove_from_hass(self):
        await super().async_will_remove_from_hass()
        if self._key != "BSH.Common.Status.RemoteControlActive":
            self._appliance.deregister_callback(
                self.async_on_update, "BSH.Common.Status.RemoteControlActive"
            )
...
class EntityManager:
    ...
    def add(self, entity: Entity) -> None:
        ...
        if (
            entity
            and (entity.unique_id not in self._existing_ids)
            and (entity.unique_id not in self._pending_entities)
        ):
