"""Implement Home Connect Number entities."""
            ...
            for program in appliance.available_programs.values():
                if program.options:
                    for option in program.options.values():
                        if option.key not in SPECIAL_ENTITIES[
                            "ignore"
                        ] and option.type in ["Int", "Float", "Double"]:
                            device = OptionNumber(
                                appliance, option.key, Configuration({"opt": option})
                            )
                            entity_manager.add(device)

        if appliance.settings:
            for setting in appliance.settings.values():
                if setting.key not in SPECIAL_ENTITIES["ignore"] and setting.type in [
                    "Int",
                    "Float",
                    "Double",
                ]:
                    device = SettingsNumber(
                        appliance, setting.key, Configuration({"opt": setting})
                    )
                    entity_manager.add(device)
...
class SettingsNumber(InteractiveEntityBase, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        try:
            ...
            raise HomeAssistantError(
                f"Failed to apply the setting value: ({ex.code})"
            ) from ex
