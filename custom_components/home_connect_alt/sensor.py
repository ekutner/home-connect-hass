...
from home_connect_async import Appliance, Events, GlobalStatus, HomeConnect
...
from .const import (
    ...
    CONF_SENSORS_TRANSLATION_SERVER,
    DEVICE_ICON_MAP,
    DOMAIN,
    HOME_CONNECT_DEVICE,
    SPECIAL_ENTITIES,
...
class ProgramSensor(EntityBase, SensorEntity):
    ...
    _attr_device_class = "{DOMAIN}__programs"
    ...
    def native_value(self):
        ...
        if prog:
            if (
                prog.name
                and self._conf[CONF_SENSORS_TRANSLATION]
                == CONF_SENSORS_TRANSLATION_SERVER
            ):
                return prog.name
            return prog.key
...
class ProgramOptionSensor(EntityBase, SensorEntity):
    ...
    def native_value(self):
        ...
        if (
            option.displayvalue
            and self._conf[CONF_SENSORS_TRANSLATION] == CONF_SENSORS_TRANSLATION_SERVER
        ):
...
class StatusSensor(EntityBase, SensorEntity):
    ...
    @property
    def native_unit_of_measurement(self) -> str | None:
        if status := self._appliance.status.get(self._key):
            return status.unit
        return None
...
class SettingsSensor(EntityBase, SensorEntity):
    ...
    def name_ext(self) -> str:
        if self._key in self._appliance.settings:
            if setting := self._appliance.settings[self._key]:
                return setting.name
        return None
    ...
    @property
    def native_unit_of_measurement(self) -> str | None:
        if setting := self._appliance.settings.get(self._key):
            return setting.unit
        return None
...
class HomeConnectStatusSensor(SensorEntity):
    ...
    _attr_unique_id = "homeconnect_status"
    _attr_name = "Home Connect Status"
